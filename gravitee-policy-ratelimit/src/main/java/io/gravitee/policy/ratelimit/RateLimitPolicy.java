...
public class RateLimitPolicy {
    ...
    private String createRateLimitKey(boolean async, Request request, ExecutionContext executionContext, int idx) {
        ...
        // 5_ RATE_LIMIT_ID
        if (async) {
            return executionContext.getComponent(Node.class).id() + KEY_SEPARATOR +
                    executionContext.getAttribute(ExecutionContext.ATTR_API) + KEY_SEPARATOR +
                    executionContext.getAttribute(ExecutionContext.ATTR_APPLICATION) + KEY_SEPARATOR +
                    executionContext.getAttribute(ExecutionContext.ATTR_RESOLVED_PATH).hashCode() + KEY_SEPARATOR +
                    idx;
        }

        return (String) executionContext.getAttribute(ExecutionContext.ATTR_API) + KEY_SEPARATOR +
                executionContext.getAttribute(ExecutionContext.ATTR_APPLICATION) + KEY_SEPARATOR +
                executionContext.getAttribute(ExecutionContext.ATTR_RESOLVED_PATH).hashCode() + KEY_SEPARATOR +
                idx;
    }
